...
const actions = [
  ...
  { id: 'log', label: 'View Log', icon: FileClock },
...
export function SettlementView() {
  ...
  const [dateType, setDateType] = useState('Payroll Date');
  ...
  usePageHeader([
    {
      ...
      icon: FileText,
      primary: true,
      onClick: () => toast(`Report generated using ${dateType}`),
    },
    {
      id: 'show-generated',
      label: 'Show Generated',
      icon: Download,
      onClick: () => toast('Showing generated reports'),
    },
    {
      id: 'bulk-email',
      label: 'Bulk Email Log',
      icon: Send,
      onClick: () => toast('Bulk email log opened'),
    },
  ]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        ...
        (!payrollDate || r.payrollDate === payrollDate) &&
        (!q || `${r.driver} ${r.driverCode} ${r.id}`.toLowerCase().includes(q)),
    );
  }, [rows, region, division, category, payrollDate, search]);
  ...
  const rowAction = (id: string, row: SettlementRow) => {
    ...
    toast(
      id === 'delete'
        ? 'Settlement deleted'
        : `${actions.find((x) => x.id === id)?.label} started for ${row.driverCode}`,
    );
  };
      ...
      <div className="mod-filters">
        <label className="mod-filter">
          ...
          <select value={dateType} onChange={(e) => setDateType(e.target.value)}>
            <option>Payroll Date</option>
            <option>Generated Date</option>
          </select>
        </label>
        <label className="mod-filter">
          <span>Region</span>
        ...
        <label className="mod-check">
          <input
            type="checkbox"
            checked={itinerary}
            onChange={(e) => setItinerary(e.target.checked)}
          />
          Show Trip Itinerary
        </label>
        <label className="mod-check">
              ...
              {filtered.map((r) => (
                <tr key={r.id}>
                  <td className="mod-action-col">
                    <RowActionMenu items={actions} onAction={(id) => rowAction(id, r)} />
                  </td>
                  <td>
                    <button className="mod-link">{r.id}</button>
                  </td>
                  <td>
