...
export function FuelView() {
  ...
  return <div className="mod-page">
    ...
    <div className="mod-table-shell"><div className="mod-table-scroll"><table className="data-table mod-table compact"><thead><tr><th>Receipt</th><th>Receipt Date</th><th>Effective</th><th>Payroll</th><th>Driver</th><th>Truck</th><th>Type</th><th>Qty L</th><th>Vendor</th><th>Location</th><th>Rate</th><th>Tax</th><th>IFTA</th><th>Deduct</th><th className="mod-action-col">Action</th></tr></thead><tbody>
      {filtered.map((r) => <tr key={r.id}><td><button className="mod-link" onClick={() => setModalRow(r)}>{r.receiptNo}</button></td><td>{r.receiptDate}</td><td>{r.effectiveDate}</td><td>{r.payrollDate}</td><td>{r.driverCode}</td><td>{r.truckNo}</td><td>{r.itemType}</td><td className="mod-money">{r.qtyLtr.toFixed(1)}</td><td>{r.vendor}</td><td>{r.cityState}</td><td className="mod-money">{money.format(r.driverRate)}</td><td className="mod-money">{money.format(r.tax)}</td><td>{r.impactIfta ? 'Yes' : 'No'}</td><td>{r.allowDeduction ? 'Yes' : 'No'}</td><td className="mod-action-col"><RowActionMenu items={actions} onAction={(id) => action(id, r)} /></td></tr>)}
