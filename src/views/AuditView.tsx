...
export function AuditView() {
  ...
  return <div className="mod-page">
    ...
    <div className="mod-table-shell"><div className="mod-table-scroll"><table className="data-table mod-table"><thead><tr><th className="mod-action-col">Action</th><th>Trip No.</th><th>Flag Status</th><th>Driver Code</th><th>Description</th><th>Type</th><th>Created On</th></tr></thead><tbody>
      {rows.map((r) => <tr key={r.id}><td className="mod-action-col"><RowActionMenu items={actions} onAction={(id) => toast(id === 'view' ? `Opening ${r.tripNo}` : `Exported ${r.tripNo}`)} /></td><td><button className="mod-link" onClick={() => toast(`Opening ${r.tripNo}`)}>{r.tripNo}</button></td><td><span className={`mod-status ${r.flagStatus}`}>{r.flagStatus}</span></td><td>{r.driverCode}</td><td>{r.description}</td><td>{r.type}</td><td>{r.createdOn}</td></tr>)}
