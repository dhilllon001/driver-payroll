...
export function SettlementView() {
  ...
  return <div className="mod-page">
    ...
    <div className="mod-table-shell"><div className="mod-table-scroll"><table className="data-table mod-table"><thead><tr><th className="mod-action-col">Action</th><th>ID</th><th>Driver</th><th>Division</th><th>Region</th><th>Category</th><th>Payroll Date</th><th>Status</th><th>Amount</th><th>Emailed</th><th>Generated</th></tr></thead>
      <tbody>{filtered.map((r) => <tr key={r.id}><td className="mod-action-col"><RowActionMenu items={actions} onAction={(id) => rowAction(id, r)} /></td><td><button className="mod-link">{r.id}</button></td><td><div className="driver-cell"><span className="name">{r.driver}</span><span className="uid">{r.driverCode}</span></div></td><td>{r.division}</td><td>{r.region}</td><td>{r.category}</td><td>{r.payrollDate}</td><td><span className={`mod-status ${r.status}`}>{r.status}</span></td><td className="mod-money">{money.format(r.amount)}</td><td>{r.emailed ? 'Yes' : 'No'}</td><td>{r.generatedAt}</td></tr>)}</tbody></table></div>
