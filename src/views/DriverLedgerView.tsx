...
export function DriverLedgerView() {
  ...
  return <div className="mod-page">
    ...
    <div className="mod-table-shell"><div className="mod-table-scroll"><table className="data-table mod-table"><thead><tr><th>Date</th><th>Driver</th><th>Description</th><th>Debit</th><th>Credit</th><th>FX Rate</th><th>FX Amount</th><th>Balance</th><th>Balance FX</th><th>Updated By</th><th className="mod-action-col">Action</th></tr></thead><tbody>
      {rows.map((r) => <tr key={r.id}><td>{r.txnDate}</td><td><div className="driver-cell"><span className="name">{r.driver}</span><span className="uid">{r.driverCode}</span></div></td><td>{r.description}</td><td className="mod-money">{r.debit ? money.format(r.debit) : '—'}</td><td className="mod-money">{r.credit ? money.format(r.credit) : '—'}</td><td className="mod-money">{r.exchangeRate.toFixed(4)}</td><td className="mod-money">{money.format(r.exchangeAmount)}</td><td className="mod-money">{money.format(r.balance)}</td><td className="mod-money">{money.format(r.balanceFx)}</td><td>{r.updatedBy}</td><td className="mod-action-col"><RowActionMenu items={actions} onAction={(id) => toast(`${id === 'view' ? 'Opening' : 'Exported'} ${r.id}`)} /></td></tr>)}
